...
export default function PurchaseAlbum() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      const response = await fetch('/api/purchase-album', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to purchase album');
      }
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-xl font-bold mb-4">🎵 Purchase Album</h2>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Enter the wallet address that will purchase the album (must have 0.01 USDC)
          </p>
        </div>
        ...
        >
          {isLoading ? 'Processing...' : 'Purchase Album (0.01 USDC)'}
        </button>
        ...
        {result && (
          <div className="p-3 bg-green-50 border border-green-200 rounded">
            <p className="text-sm text-green-800 font-medium">✅ Album Purchased Successfully!</p>
            <div className="mt-2 text-xs space-y-1">
              <p><strong>Transaction:</strong> {result.txHash?.slice(0, 10)}...{result.txHash?.slice(-8)}</p>
              <p><strong>Buyer:</strong> {result.buyer}</p>
              {result.albumIndex !== undefined && (
                <p><strong>Album Index:</strong> {result.albumIndex}</p>
              )}
              <a 
